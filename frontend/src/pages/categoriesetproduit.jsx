...
const INITIAL_CATEGORIES = ["Aresso jewellery full set", "aresso jewellery KLP 1", "kelepçe kataloğu", "Mini set", "MURSIT ABI FULL SET", "Teklemer", "MSNT-mını set", "panter kataloğu", "YK-yaylı bilezik", "YZ 1-Yüzük kataloğu", "YZ 2-Yüzük kataloğu", "YZK-Yuzuk katalogu", "ZINCIR"];
const INITIAL_SUBCATEGORIES = {
  "Aresso jewellery full set": [],
  "aresso jewellery KLP 1": [],
  "kelepçe kataloğu": [],
  "Mini set": [],
  "MURSIT ABI FULL SET": [],
  "Teklemer": [],
...
// helper: format filename into product/category display name
function formatProductName(raw) {
  if (!raw) return "";
  let s = raw.toString();
  s = s.replace(/\.[^.]+$/, ""); // remove extension
  s = s.replace(/[-_]+/g, ' '); // dashes/underscores -> spaces
...
export default function App() {
  ...
  const [assetsMap, setAssetsMap] = useState({}); // key: folder name -> [{src,name,path}]
  const [productsByCategory, setProductsByCategory] = useState([]); // structured output
  ...
  // scan frontend/assets for images and build productsByCategory
  useEffect(() => {
    // Vite scanne tous les fichiers images dans frontend/assets/
    const files = import.meta.glob(
      '/assets/**/*.{png,jpg,jpeg,webp,gif,PNG,JPG,JPEG}',
      { eager: true }
    );

    console.log('Fichiers trouvés:', Object.keys(files).length); // debug
    ...
    Object.entries(files).forEach(([path, module]) => {
      ...
      const category = parts[assetsIdx + 1]; // nom du dossier
      const fileName = parts[parts.length - 1];
      const productName = fileName.replace(/\.[^.]+$/, ''); // sans extension
      const src = module.default;

      if (!map[category]) map[category] = [];
      map[category].push({ src, name: productName, path });
    });
    ...
    setProductsByCategory(structured);
  ...
  const handleCategoryClick = (cat) => {
    // Si on clique sur la même catégorie → désélectionner
    ...
    const entry = productsByCategory.find(p => p.category === cat);
    ...
    // Convertir les images en produits
    const generated = entry.items.map((it, idx) => ({
      id: `img-${cat}-${idx}-${it.name}`,
      ref: it.name,
      name: it.name,
      stock: 1,
      category: cat,
      subCategory: "",
      image: it.src, // URL résolue par Vite
    }));
  ...
  return (
    <div className="flex h-screen bg-[#F4F2F5] overflow-hidden" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-[#F4F2F5] border-b border-gray-200 px-8 py-4 shrink-0">
          <h1 className="text-xl italic font-medium text-gray-800 tracking-wide" style={{ fontFamily: "'Playfair Display', serif" }}>Catégories et Stock</h1>
        </header>

        <div className="flex flex-1 min-h-0">

          
          <div className="w-56 bg-[#F4F2F5] border-r border-gray-200 overflow-y-auto shrink-0 p-4">
          ...
          <main className="flex-1 overflow-y-auto p-6 bg-[#F4F2F5]">
